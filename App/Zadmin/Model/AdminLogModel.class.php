...
class AdminLogModel extends Model {
    ...
    public function get_admin_log_page($page,$where){
        ...
        $base_purl = __ROOT__.'/index.php?m=zadmin&c=admin&a=admin_log_list&admin_id='.$page['admin_id'].'&start_time='.$page['start_time'].'&end_time='.$page['end_time'].'&ip_address='.$page['ip_address'];
	...
	public function admin_log($info,$admin_id='',$ip_address=''){
		if(empty($admin_id)) $admin_id = $_SESSION['zadmin']['info']['admin_id'];
		$data['admin_id'] = $admin_id;
		$data['log_info'] = $info;
		if(empty($ip_address)) $ip_address = $_SESSION['zadmin']['info']['ip_address'];
